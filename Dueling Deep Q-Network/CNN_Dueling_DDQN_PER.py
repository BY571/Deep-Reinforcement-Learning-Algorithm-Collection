...
from PrioritizedExperienceReplay import PrioritizedReplay
...
def compute_td_loss(batch_size,current_model,target_model,opti,loss_func,gamma,PER):
    state, action, reward, next_state, done,idx,weights = PER.sample(batch_size)
    ...
    weights    = Variable(torch.FloatTensor(weights)) #shape [32]
    ...
    loss = loss_func(q_value,Variable(expected_q_value.data))*weights
    prios = loss + 1e-5
    loss = loss.mean()
    ...
    opti.zero_grad()
    loss.backward()
    PER.update_priorities(idx, prios.data.cpu().numpy())
    opti.step()
    return loss
...
def main():
    plt.ion()
    env = wrapper.make_atari("BreakoutNoFrameskip-v4", monitor=True,epidsode_capture=50)#Riverraid Frostbite Enduro
...
#DeepMind took optim.RMSprop(current_model.parameters(), lr=0.000)
    ...
    PER = PrioritizedReplay(100000,alpha = 0.6,beta_start =0.4,beta_frames=1000000)
    ...
    for frame_idx in range(1, num_frames + 1):
        ...
        print("Training :: Frame {} :: Epsilon {} :: Reward {} ".format(frame_idx, round(epsilon,2),reward))
        # Manuel Stacking
        ...
        if PER.__len__() > replay_initial:
            loss = compute_td_loss(batch_size,current_model, target_model,opti,loss_func,gamma,PER)
