...
# EDIT BY ATAMAI
# Preparing image received from environment and adjust it to expected format of Pytorch 
# HWC (height x width x channel) becomes CHW
...
def make_atari(env_id, monitor = False, epidsode_capture = 75):
    env = gym.make(env_id)
    if monitor == True:
        env = wrappers.Monitor(env, "Videos/", resume=True, force =True, video_callable=lambda episode_id: episode_id%epidsode_capture==0)   
